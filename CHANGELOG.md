...
- The sidebar and the chat pane are outlined with full rounded borders (`╭╮╰╯` on all four corners of each pane): the sidebar's divider column became its right border (a left border now runs down the frame edge) and the chat header's top-border row became the chat pane's top border, with a bottom border added above the spinner line. Titles, the status-strip readout, focus emphasis, and the mouse hit-test geometry are unchanged; the chat content area is two columns narrower and one row shorter inside the new borders.
